<template>
  <div>
    <b-card :title="'Edit brand with id of ' + id">
      ...
      <b-button variant="success" @click="saveCelloBrand">Save</b-button>
    </b-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    getBradById() {
      this.axios
        .get("https://localhost:5001/api/CelloBrands/" + this.id)
        .then((response) => {
    ...
    saveCelloBrand() {
      ...
      this.axios
        .put("https://localhost:5001/api/CelloBrands/" + this.id,
         params)
        .then(() => {
          alert("success");
          this.$router.push("/celloBrands");
        })
        .catch(() => {
          alert("failed");
        });
